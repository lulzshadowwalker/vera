function d() {
  return {
    ...
    toggleSelectRecordsInGroup: async function (e) {
      this.isLoading = !0
      let t = await this.$wire.getGroupedSelectableTableRecordKeys(e)
      ;(this.areRecordsSelected(this.getRecordsInGroupOnPage(e))
        ? this.deselectRecords(t)
    ...
    handleCheckboxClick: function (e, t) {
      ...
      if (e.shiftKey) {
        ...
        let l = s.indexOf(this.lastChecked),
          r = s.indexOf(t),
          o = [l, r].sort((c, n) => c - n),
          i = []
        for (let c = o[0]; c <= o[1]; c++)
          ((s[c].checked = t.checked), i.push(s[c].value))
        t.checked ? this.selectRecords(i) : this.deselectRecords(i)
      }
      this.lastChecked = t
    },
  }
