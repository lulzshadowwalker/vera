<header class="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
    <div class="container mx-auto flex h-14 items-center px-4 md:px-6">
        <!-- Mobile Menu Trigger -->
        <button type="button" class="btn-outline btn-icon md:hidden mr-2" onclick="document.dispatchEvent(new CustomEvent('basecoat:sidebar', { detail: { id: 'mobile-sidebar' } }))">
            <i class="hgi hgi-stroke hgi-menu-01 text-xl text-foreground"></i>
            <span class="sr-only">Toggle Menu</span>
        </button>

        <!-- Logo -->
        <a href="{{ route('home.index') }}" class="mr-6 flex items-center space-x-2">
            <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" class="h-6 w-6 text-primary" xmlns="http://www.w3.org/2000/svg">
            ...
            <span class="hidden font-bold sm:inline-block text-xl">{{ config('app.name') }}</span>
        </a>
