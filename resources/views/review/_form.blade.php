...
<div x-data="{
    ...
    <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            <!-- Deal Date -->
            <div>
                <label class="mb-2 block text-sm font-semibold" for="deal_date">
                ...
                <p x-show="!isDealDateValid" x-cloak class="text-destructive mt-2 flex items-start gap-1 text-sm">
                    <i class="hgi hgi-stroke hgi-alert-square"></i>
                    Deal date cannot be more than 3 years old{{ isset($review) ? ' relative to the review date' : '' }}.
                </p>
                @error('deal_date')
        ...
        <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
            @foreach ([
                ...
                <div x-data="{ value: {{ old($metric, $review->$metric ?? 5) }} }">
                    <label class="mb-3 block min-h-19" for="{{ $metric }}">
                    ...
                    <div class="flex items-center gap-4">
                        <input type="range"
                               ...
                               class="input w-full"
                               required>
                        <span class="w-8 text-center text-lg font-bold" x-text="value"></span>
                    </div>
                    @error($metric)
                        <p class="text-destructive mt-2 flex items-center gap-1 text-sm">
                            <i class="hgi hgi-stroke hgi-alert-square"></i>
            ...
            <div class="shrink-0">
                <label class="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox"
                           ...
                           x-model="anonymous"
                           class="peer sr-only">
                    <div class="bg-muted peer-focus:ring-primary/30 peer-checked:bg-primary peer h-6 w-11 rounded-full after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:outline-none peer-focus:ring-4">
                    </div>
                </label>
            </div>
        </div>
        <div class="bg-muted mt-4 rounded-lg p-3" x-show="anonymous">
    ...
    <div class="card p-6">
        <div>
            <h3 class="mb-2 flex items-center gap-2 text-lg font-bold">
                ...
                Additional Comments
                <span class="text-muted-foreground text-sm font-normal" x-show="anonymous">(disabled for anonymous reviews)</span>
            </h3>
        ...
        <div>
            <textarea class="textarea @error('comment') border-destructive @enderror w-full"
                      ...
                      :disabled="anonymous"
                      x-model="comment"
                      placeholder="Example: Great communication and fast delivery. Would recommend for urgent projects."></textarea>
    ...
    <div class="card bg-muted/50 p-6">
        <div class="flex flex-col items-center justify-between gap-4 md:flex-row">
            ...
            <div class="flex w-full flex-col gap-3 sm:flex-row md:w-auto">
                <a href="{{ route('suppliers.show', isset($review) ? $review->reviewedSupplier : $supplier) }}"
